...
use crate::client::{
    components::player_component::MiniMapPlayer,
...
pub fn spawn_mini_map_player(mut commands: Commands, res_player: &Res<PlayerResource>) {
    ...
    commands.spawn((
        Sprite {
            color: Color::BLACK,
            custom_size: Some(Vec2::new(shape_size * 0.5, shape_size * 0.5)),
            ..Default::default()
        },
        Transform::from_xyz(
...
pub fn update_minimap_player(
    ...
    if let Ok(mut minimap_transform) = minimap_query.get_single_mut() {
        ...
        let minimap_x = (res_player.position.x * 1.0) - 0.0; // 🔄 Échelle arbitraire (à ajuster)
        let minimap_y = (res_player.position.z * 1.0) - 0.0;
